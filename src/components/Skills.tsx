...
const skillGroups = [
  { label: 'Frontend', icon: '\u{26A1}', color: '#00d4ff', skills: ['Next.js', 'React', 'TypeScript', 'Tailwind CSS', 'Framer Motion', 'HTML/CSS'] },
  { label: 'Backend', icon: '\u{1F527}', color: '#10b981', skills: ['Node.js', 'PostgreSQL', 'Supabase', 'REST APIs', 'WebSockets', 'Edge Functions'] },
  { label: 'Game Dev', icon: '\u{1F3AE}', color: '#7c3aed', skills: ['Unity', 'C#', 'Game Design', 'Pixel Art', 'Sprite Animation', 'RPG Systems'] },
  { label: 'DevOps & Tools', icon: '\u{1F680}', color: '#f59e0b', skills: ['Vercel', 'GitHub', 'Railway', 'Render', 'Netlify', 'CI/CD'] },
  { label: 'Business & SaaS', icon: '\u{1F4BC}', color: '#ec4899', skills: ['Stripe', 'Multi-tenant Architecture', 'OAuth', 'Analytics', 'QR Systems'] },
  { label: 'AI & Integrations', icon: '\u{1F916}', color: '#06b6d4', skills: ['Claude', 'OpenAI API', 'LLM Integration', 'Automation', 'Webhooks', 'Perplexity'] },
];
          ...
          {skillGroups.map((g, i) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4 sm:mb-5">
                <div className="skill-icon-box w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-lg sm:text-xl" style={{ background: `${g.color}12`, border: `1px solid ${g.color}22`, color: g.color }}>{g.icon}</div>
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {g.skills.map((s, j) => (
                  <motion.span key={s} initial={{ opacity: 0, scale: 0.8 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.1 + i * 0.05 + j * 0.03 }}
                    className="text-xs font-mono px-2.5 py-1.5 rounded-lg border cursor-default hover:scale-110 transition-transform duration-200"
